...
export function chatControl(text: string): { kind: 'continue' | 'commit' | 'pending' | 'ignore'; body: string } {
  const t = text.trim()
  if (t.endsWith('..')) {
...
export class SessionMerger {
  ...
  constructor(private readonly opts: MergerOptions) {}
  ...
  private rearm(userId: string): void {
    const old = this.timers.get(userId)
    if (old) clearTimeout(old)
    const t = setTimeout(() => {
      const text = this.drain(userId)
      if (text) this.opts.onTimeoutFlush(userId, text)
    }, this.opts.mergeTimeoutMs)
    t.unref?.()
    this.timers.set(userId, t)
  }

  private drain(userId: string): string {
    const t = this.timers.get(userId)
    if (t) clearTimeout(t)
    this.timers.delete(userId)
    const buf = this.buffers.get(userId) ?? []
    this.buffers.delete(userId)
    if (buf.length > 0) this.opts.onSnapshot(userId, [])
    return buf.join('\n')
  }
