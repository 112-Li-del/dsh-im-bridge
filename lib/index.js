...
import { reply, runBridgeLoop } from './bridge.js';
...
export function apply(ctx, config = {}) {
    const cfg = {
        ...
        statePath: config.statePath ?? join(process.env.DSH_HOME ?? join(homedir(), '.dsh'), 'dsh-im-bridge', 'state.json'),
    };
    ...
    const boundSessionId = () => store.boundSessionId || lastActiveSessionId;
    /** 推送到微信（只能发给白名单用户；没有 context_token 时只能记日志） */
    const pushToWechat = async (text) => {
    ...
    const lastAssistantText = (session) => {
        for (const ev of [...session.events].reverse()) {
            ...
            const text = ev.data.message.content
                ...
                .trim();
            if (text)
                return [...text].slice(0, LAST_TEXT_SNIPPET_CHARS).join('');
        }
        return '';
    };
    ...
    ctx.on('session/event', (session, event) => {
        ...
        if (event.type !== 'turn/end')
            return;
        if (String(session.id) !== boundSessionId())
            return;
        const label = TURN_END_LABEL[event.data.reason.kind] ?? event.data.reason.kind;
        const snippet = lastAssistantText(session);
        void pushToWechat(`[${label}] 会话 ${String(session.id)} 第 ${event.data.turn} 轮结束` + (snippet ? `\n${snippet}` : '')).catch((err) => log.warn('turn/end 推送失败:', err));
    }, { global: true });
    ...
    ctx.on('approval/request', async (req, next) => {
        ...
        if (!store.allowedUserId)
            return next();
        if (String(req.agent.session.id) !== boundSessionId())
            return next();
        await pushToWechat(`⚠️ 批准请求\n工具：${req.toolName}\n原因：${req.reason ?? '（未给出）'}\n回复「批准」或「拒绝」（${cfg.approvalTimeoutSecs}s 内有效，超时转回本机批准体系）`).catch((err) => log.warn('approval 推送失败:', err));
    ...
    const injectToSession = async (text) => {
        const sid = boundSessionId();
        if (!sid)
            return '还没有活跃会话。请先在 DSH 里开始一个任务，或用 /bind <session-id> 绑定。';
        const agent = ctx.agents.get(sid);
        if (!agent)
            return `会话 ${sid} 当前没有运行中的 agent，无法注入。`;
    ...
    const merger = new SessionMerger({
        ...
        onTimeoutFlush: (userId, text) => {
            void injectToSession(text).then(async (ack) => {
                if (ack)
                    await pushToWechat(ack);
            });
        },
    });
    // 崩溃恢复：重启后把未 flush 的缓冲恢复（last_update=now，超时后即注入）
    for (const [userId, buf] of Object.entries(store.mergeBuffers()))
        merger.restore(userId, buf);
    const HELP = [
        '可用命令：',
    ...
    bridgeOpts.onMessage = async (msg) => {
        ...
        switch (r.kind) {
            ...
            case 'bind': {
                const s = ctx.sessions.get(r.sessionId);
                if (!s)
                    return `会话 ${r.sessionId} 不存在（当前活跃：${lastActiveSessionId || '无'}）。`;
            ...
            case 'chat': {
                ...
                if (action.kind === 'flush')
                    return injectToSession(action.text);
    ...
    ctx.jobs.start({
        ...
        run: () => {
            ...
            const done = runBridgeLoop(bridgeOpts, abort.signal).then(() => ({ status: 'completed' }), (err) => ({ status: 'failed', detail: String(err) }));
            return {
                cancel: () => {
                    abort.abort();
                    merger.dispose();
