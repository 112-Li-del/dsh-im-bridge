...
export async function login(opts, signal) {
    ...
    while (!signal.aborted) {
        ...
        while (!signal.aborted) {
            ...
            try {
                ...
                if (!isTimeout(err))
                    log.warn('ilink 查询扫码状态失败，2s 后重试:', err);
...
export async function reply(opts, toUserId, text, contextToken) {
        ...
        while (!signal.aborted) {
            ...
            for (const msg of page.messages) {
                if (signal.aborted)
                    return;
                try {
                    if (store.checkAndMark(msg.messageId))
                        continue;
                    if (msg.contextToken)
                        store.updateContextToken(msg.fromUserId, msg.contextToken);
                    ...
                    if (out !== undefined)
                        await reply(opts, msg.fromUserId, out, msg.contextToken);
                }
                catch (err) {
